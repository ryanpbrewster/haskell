...
countUglyExpressions s = let possible_sums = makeAllExpressions s
                         in length $ filter isUgly possible_sums

makeAllExpressions (x:xs) = makeAllExpressions' xs x
    where makeAllExpressions' [] acc = [acc]
          makeAllExpressions' (x:xs) acc =
              let useit = makeAllExpressions' xs (10*acc + x) -- append
                  loseit = makeAllExpressions' xs x           -- + or -
              in useit ++ map (acc+) loseit ++ map (acc-) loseit
