module Problems.P147 (solve) where
...
crossHatchRects wb hb = (straight_rects, corner_rects, center_rects)
    where straight_rects = [ (i,j,w,h) | i <- [0..hb-1]
                                       , j <- [0..wb-1]
                                       , let w_bound = wb-j
                                       , w <- [1..w_bound]
                                       , let h_bound = hb-i
                                       , h <- [1..h_bound]]
          corner_rects = [ (i,j,w,h) | i <- [0..hb-1]
                                     , j <- [1..wb-1]
                                     , let w_bound = min (2*wb-2*j) (2*hb-2*i-1)
                                     , w <- [1..w_bound]
                                     , let h_bound = min (2*j) (2*hb-2*i-w)
                                     , h <- [1..h_bound]]
          center_rects = [ (i,j,w,h) | i <- [0.5..hb-0.5]
                                     , j <- [0.5..wb-0.5]
                                     , let w_bound = min (2*wb-2*j) (2*hb-2*i-1)
                                     , w <- [1..w_bound]
                                     , let h_bound = min (2*j) (2*hb-2*i-w)
...
bruteForce wb hb = let (straight, corner, center) = crossHatchRects wb hb
