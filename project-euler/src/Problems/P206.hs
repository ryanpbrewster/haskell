...
isWildPrefixOf (t:ts) (g:gs)
  | t == 10 = isWildPrefixOf ts gs
  | otherwise = (t == g) && isWildPrefixOf ts gs

findMatches [] = [0]
findMatches ts@(t:ts') =
  let ms = findMatches ts'
      offset = 10 ^ (length ts')
      candidates = [offset * d + m | d <- [0 .. 9], m <- ms]
  in filter legit candidates
  where
    legit v = matchesTemplate ts (integerDigits (v ^ 2))

solveProblem =
  let template = intersperse 10 [1, 2, 3, 4, 5, 6, 7, 8, 9]
